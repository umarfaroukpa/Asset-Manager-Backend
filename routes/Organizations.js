...
const router = express.Router();// @route   POST /api/organizations
// @desc    Create new organization
// @access  Private
router.post('/', authenticateToken, async (req, res) => {
    try {
        const { name, type, description } = req.body;    // Check if user already has an organization as owner
...
router.get('/my', authenticateToken, async (req, res) => {
    try {
        ...
        .populate('members.user', 'name email');    if (!organization) {
...
router.put('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const organizationId = req.params.id;    const organization = await Organization.findById(organizationId);
    if (!organization) {
        return res.status(404).json({
            success: false,
            message: 'Organization not found'
...
router.post('/:id/invite', authenticateToken, async (req, res) => {
    try {
        ...
        const organizationId = req.params.id;    // Find organization and check permissions
...
router.put('/:id/members/:userId', authenticateToken, async (req, res) => {
    try {
        ...
        const { id: organizationId, userId } = req.params;    const organization = await Organization.findById(organizationId);
    if (!organization) {
        return res.status(404).json({
            success: false,
            message: 'Organization not found'
...
router.get('/:id/members', authenticateToken, async (req, res) => {
    try {
        const organization = await Organization.findById(req.params.id)
            .populate('members.user', 'name email createdAt');    if (!organization) {
        return res.status(404).json({
            success: false,
            message: 'Organization not found'
        });
...
router.delete('/:id', authenticateToken, async (req, res) => {
    try {
        const organization = await Organization.findById(req.params.id);    if (!organization) {
        return res.status(404).json({
            success: false,
            message: 'Organization not found'
        });
